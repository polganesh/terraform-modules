# it is important that all public subnets must be tagged with following keys. these keys help EKS to create  external load balancer in public subnet
...
resource "null_resource" "tag-privateApp-subnet" {
  ...
  provisioner "local-exec" {
    command = "aws ec2 create-tags --resources ${element(data.aws_subnet_ids.private_app_subnets.ids, count.index)} --tags Key=kubernetes.io/cluster/eks-${var.region_id}-${var.environment}-${var.cost_centre}-vpc${var.vpc_seq_id}-${var.app_service}-${var.seq_id},Value=shared"
  }
